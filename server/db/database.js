...
// One-shot migrations live outside the migrations array (which re-runs every boot) and
// record themselves in schema_migrations instead. runOnce owns that check-then-run-
// then-record skeleton so the copies can't drift. fn(db) returns the rows touched (for
// the log) and may throw: a failure leaves the id unrecorded so the next boot retries.
function runOnce(id, label, fn) {
  try {
    if (db.prepare('SELECT 1 FROM schema_migrations WHERE id = ?').get(id)) return false;
    const changed = fn(db) || 0;
    db.prepare('INSERT OR IGNORE INTO schema_migrations (id) VALUES (?)').run(id);
    if (changed > 0) console.log(`[migrate] ${label}: ${changed} row(s)`);
    return true;
  } catch (e) {
...
runOnce('offline_alert_per_outage_backfill', 'offline-alert backfill (marked already-alerted)', (db) =>
  db.prepare(`UPDATE devices SET offline_alert_heartbeat = last_heartbeat
...
// id-preserving save prevents recurrence.
{
  ...
  runOnce(DEDUPE_ID, 'removed duplicate template zone', (db) => db.prepare(`
...
// Content-only schedules invent a throwaway one-item playlist, but the insert omitted
// is_auto_generated — so the playlists page "Show auto-generated" toggle (which filters
// on that flag) could never hide them. New rows are flagged at insert time
// (createAutoGeneratedPlaylist in server/lib/auto-playlist.js); this one-shot backfills
// rows that pre-date the fix. Structural selector (the schedule's own playlist pointer),
// never the display name: names are user-editable in both directions.
runOnce('backfill_scheduled_playlists_auto_generated_v1', 'flagged Scheduled playlist as auto-generated',
  require('../lib/auto-playlist').backfillScheduledPlaylists);
