...
class SystemControl(private val context: Context) {
    ...
    /** System-wide brightness, [fraction] 0..1. Disables auto-brightness first. Needs WRITE_SETTINGS. */
    fun setSystemBrightness(fraction: Double): Boolean {
        ...
        return try {
            ...
            Settings.System.putInt(cr, Settings.System.SCREEN_BRIGHTNESS,
                (fraction.coerceIn(0.0, 1.0) * 255).roundToInt().coerceIn(0, 255))
            true
        } catch (e: Throwable) { Log.w(TAG, "setSystemBrightness: ${e.message}"); false }
    }

    /** Screen-off timeout in [ms] (<= 0 = never / Int.MAX_VALUE). Needs WRITE_SETTINGS. */
    fun setScreenOffTimeout(ms: Int): Boolean {
        if (!canWriteSettings()) return false
        return try {
            Settings.System.putInt(context.contentResolver, Settings.System.SCREEN_OFF_TIMEOUT,
                if (ms <= 0) Int.MAX_VALUE else ms)
            true
